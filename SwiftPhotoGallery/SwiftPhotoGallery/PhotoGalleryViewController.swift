...
class PhotoGalleryViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        var imagePath = REMOTE_DATA_FOLDER_PATH + photo.thumbImage
    ...
    private func loadPhotoGallery() -> Void {
        ...
        self.fetchJsonData(
            ...
            onCompletion: { (jsonData: JSON) -> Void in
                ...
                for (index: String, subJson: JSON) in photosJSON {
    ...
    func downloadCellPhotoInBackground( #imageUrl: NSURL, photoCell: PhotoCollectionViewCell ) {
    ...
    private func calculateCellWidthHeight() {
        ...
        var screenWidth = self.photoCollectionView.frame.width
        
        // deduct spacing from screen width
        // Formula: screeWidth - leftSpace - ( spaceBetweenThumb * (numberOfPhotoInRow - 1) ) - rightSpace
        var netWidth = screenWidth - 5 - ( 5 * (numberOfPhotoInRow - 1) ) - 5
        
        // calcualte single thumb width
        var thumbWidth = Int( netWidth / numberOfPhotoInRow)
    ...
    private func fetchJsonData(#jsonFilePath: String, onCompletion: (jsonData: JSON) -> Void, onError: () -> Void ) {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "segueSinglePhoto" {
            ...
            let clickedIndexPath: [NSIndexPath] = self.photoCollectionView!.indexPathsForSelectedItems() as! [NSIndexPath]
            
            // create destination view controller
            var destViewCtrl = segue.destinationViewController as! SinglePhotoViewController
